...
pub fn new_aes_128_gcm_bicrypter(key: &Key128Bits) -> AesNonceBicrypter<Aes128Gcm> {
    let key = GenericArray::clone_from_slice(key);
    AesNonceBicrypter::new(Aes128Gcm::new(key))
}

pub fn new_aes_256_gcm_bicrypter(key: &Key256Bits) -> AesNonceBicrypter<Aes256Gcm> {
    let key = GenericArray::clone_from_slice(key);
    AesNonceBicrypter::new(Aes256Gcm::new(key))
}
...
mod tests {
    ...
    fn aes_256_gcm_bicrypter_can_encrypt_and_decrypt() {
        let bicrypter = new_aes_256_gcm_bicrypter(b"some 256-bit (32-byte) key------");
