...
pub fn new_aes_128_gcm_siv_bicrypter(
    key: &Key128Bits,
) -> AesNonceBicrypter<Aes128GcmSiv> {
    let key = GenericArray::clone_from_slice(key);
    AesNonceBicrypter::new(Aes128GcmSiv::new(key), NonceSize::Nonce96Bits)
}

pub fn new_aes_256_gcm_siv_bicrypter(
    key: &Key256Bits,
) -> AesNonceBicrypter<Aes256GcmSiv> {
    let key = GenericArray::clone_from_slice(key);
...
mod tests {
    ...
    fn aes_256_gcm_siv_bicrypter_can_encrypt_and_decrypt() {
        let bicrypter =
            new_aes_256_gcm_siv_bicrypter(b"some 256-bit (32-byte) key------");
